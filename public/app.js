...
let configuration = { maxCharacters: 4096, defaultLanguage: "French", presets: [] };
...
function renderConfig(config) {
  ...
  appendOptions(elements.dialogueVoiceA, config.presets, "wizard", presetLabel);
  appendOptions(elements.dialogueVoiceB, config.presets, "goat", presetLabel);
  ...
  try {
    const response = await fetch("/api/speech", {
      ...
      body: JSON.stringify({
        ...
        voiceDescription: elements.description.value,
    ...
    elements.meta.textContent = `${elements.language.value.toUpperCase()} · LOCAL · ${(blob.size / 1024).toFixed(0)} KO · ${((performance.now() - startedAt) / 1000).toFixed(1)} S`;
...
elements.dialogueForm.addEventListener("submit", async (event) => {
  ...
  const voiceA = configuration.presets.find((preset) => preset.id === elements.dialogueVoiceA.value);
  const voiceB = configuration.presets.find((preset) => preset.id === elements.dialogueVoiceB.value);
  if (!voiceA || !voiceB) {
  ...
  try {
    const response = await fetch("/api/dialogue", {
      ...
      body: JSON.stringify({
        ...
        voiceADescription: voiceA.description,
        voiceBDescription: voiceB.description,
